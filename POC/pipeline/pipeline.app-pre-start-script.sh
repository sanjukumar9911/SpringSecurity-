...
if serviceEnabled "$CONFIGSERVER_ENABLED" && serviceEnabled "$CONFIGSERVER_CONFIGURED" && serviceExists 'vdcc-config-server' ; then
  cf bind-service "%BOOST_STAGE_APP_NAME%" 'vdcc-config-server'
fi
